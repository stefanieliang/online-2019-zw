...
export default {
  ...
  filters: {
    formatComponentName: componentName =>
      componentName.replace(/^\w/g, name => name.toUpperCase())
  },
  ...
  created() {
    this.getListData(this.curTab).then(listData => {
  ...
  methods: {
    ...
    switchTab(tabName) {
      ...
      if (!this.tabs[tabName].list.length) {
        this.getListData(tabName).then(listData => {
          this.setTabsData(this.curTab, {
            list: listData
          });
        });
